...
def superlet(
    ...
    if adaptive:
        ...
        gmean_spec = fract_adapt_slt(data_arr, samplerate, scales, order_max, order_min, c_1)

    # multiplicative SLT
    else:

        gmean_spec = multiplicative_slt(
...
def multiplicative_slt(data_arr, samplerate, scales, order_max, order_min=1, c_1=3):
    dt = 1 / samplerate
    ...
    # lowest order
    gmean_spec = cwt_sl(data_arr, sl[0], scales, dt)
    gmean_spec = np.power(gmean_spec, 1 / order_num)

    for wavelet in sl[1:]:
        spec = cwt_sl(data_arr, wavelet, scales, dt)
        gmean_spec *= np.power(spec, 1 / order_num)

    return gmean_spec


def fract_adapt_slt(data_arr, samplerate, scales, order_max, order_min=1, c_1=3):
    ...
    gmean_spec = cwt_sl(data_arr, sl[0], scales, dt)  # 1st order <-> order_min
    ...
    for i, jump in enumerate(order_jumps):
        ...
        # order + 1 spec
        next_spec = cwt_sl(data_arr, sl[i + 1], scales_o, dt)
...
def cwt_sl(data, wavelet, scales, dt):
    """
    ...
    # 1st axis is time
    slices = np.array([None for _ in data.shape])
    slices[0] = slice(None)
